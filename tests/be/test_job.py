...
def test_custom_init():
    ...
    job = MyJob()
    assert job.name == "tests.be.test_job.MyJob"
    assert job.author == "mra"
...
def test_config_cascade():
    ...
    assert job.config.tests.be.test_job.MyJob.my_var == "local value"
    assert "unknown" not in job.config.tests.be.test_job.MyJob
