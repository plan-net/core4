...
def test_general_url_restriction(http):
    ...
    response = http.post("/core4/api/setting", json=data, base=False)
    assert response.ok
    assert response.json()["data"]["_general"]["language"] == "UA"

    response = http.get("/core4/api/setting/_general//language", base=False)
    assert response.status_code == 400


def test_response_contains_data(http):
    to_send = {"_general": {"language": "UA"}}
    response = http.post("/core4/api/setting", json={"data": to_send}, base=False)
    assert response.ok
...
def test_levels_restrictions(http):
    ...
    # empty key in level2
    to_send = {"": {"language": "UA"}}
    response = http.post("/core4/api/setting/_general", json={"data": to_send}, base=False)
    assert response.status_code == 400

    # invalid level1 structure
    response = http.post("/core4/api/setting", json={"data": 123}, base=False)
    assert response.status_code == 400

    # invalid level2 structure
    response = http.post("/core4/api/setting/_general", json={"data": 123}, base=False)
    assert response.status_code == 400
    ...
    response = http.post("/core4/api/setting/_project", json={"data": to_send}, base=False)
    assert response.status_code == 400

    # empty resource in url
    to_send = {"code": "UA"}
    response = http.post("/core4/api/setting//language", json={"data": to_send}, base=False)
    assert response.status_code == 400


def test_get_default_setting(http):
    response = http.get("/core4/api/setting", base=False)
    assert response.ok
...
def test_delete_settings(http):
    ...
    response = http.delete("/core4/api/setting", base=False)
    assert response.ok
...
def test_delete_nested_settings(http):
    ...
    response = http.delete("/core4/api/setting/project/setting1", base=False)
    assert response.ok
    ...
    response = http.delete("/core4/api/setting/project", base=False)
    assert response.ok
...
def test_user_override(http):
    ...
    response = http.post("/core4/api/setting/test_project/test_setting?username=test", json={"data": test_data}, base=False)
    assert response.ok

    original_token = http.token
    http.token = None
    response = http.get('/core4/api/login?username=test&password=password', base=False)
    assert response.ok
    http.token = response.json()["data"]["token"]

    response = http.get("/core4/api/setting/test_project/test_setting", base=False)
    assert response.ok
...
def test_set_system_setting(http):
    data = {"data": 123}
    response = http.post("/core4/api/setting/_project", json=data, base=False)
    assert response.json()["code"] == 400

    data = {"data": 123}
    response = http.post("/core4/api/setting/_general", json=data, base=False)
    assert response.json()["code"] == 400
...
def test_update_settings(http):
    ...
    response = http.put("/core4/api/setting/_general/language", json={"data": "RU"}, base=False)
    assert response.ok
...
def test_create_nested_settings(http):
    ...
    response = http.post("/core4/api/setting/_general/language", json={"data": {"name": "Ukraine"}}, base=False)
    assert response.ok

    response = http.get("/core4/api/setting/_general", base=False)
    assert response.ok
...
def test_empty_string_as_key(http):
    ...
    response = http.post("/core4/api/setting/_general/language", json={"data": {"": "Ukraine"}}, base=False)
    assert response.status_code == 200
...
def test_get_nested_settings(http):
    ...
    response = http.get("/core4/api/setting/_general/language/code", base=False)
    assert response.ok
...
def test_update_nested_settings(http):
    ...
    response = http.put("/core4/api/setting/_general/language/code", json={"data": "RU"}, base=False)
    assert response.ok
    ...
    response = http.put("/core4/api/setting/_general/language/name", json={"data": "Russia"}, base=False)
    assert response.ok
